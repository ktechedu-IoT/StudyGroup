...
# __str__메소드와 __repr__메소드를 부모클래스인 Person에 넣는것이 
# Logic상  이상하고 출력도 예상치 못한 결과가 나와서 자식클래스인 Member안으로 이전 했습니다.
# 그 결과 정상출력이 이루어졌습니다.
# 전의 프로그램을 무시하고 새로 보낸 프로그램 참조하세요~^^
...
class Person:
    def __init__(self, name, add, tel):
        self.name = name
        self.add=add
        self.tel=tel
